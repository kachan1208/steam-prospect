...
-- drill-down excerpt always matches what the aggregate bar shows. The excerpt arms themselves
-- render from the single source of truth ASPECT_LEXICON in build_marts.py (generated into
-- the ASPECT_REVIEW_ARMS placeholder at render time, keyword regexes included) — the same lexicon that mines
-- the vote flags AND the text-sentiment windows, so a keyword change in one place moves
-- everything together.
--
...
--
-- THE TEN ARMS ARE GENERATED (2026-09-01): the ASPECT_REVIEW_ARMS placeholder (spelled without the @@ here ON PURPOSE — render()
-- substitutes inside comments too, and naming it with delimiters injected 20KB of SQL into
-- this comment: line 1 stayed commented, every later line escaped and became live SQL)
-- expands at render time in
-- build_marts.py (_aspect_excerpt_arms_sql) into one arm per ASPECT_LEXICON entry — the SAME
-- single source of truth the keyword regexes render from. Until then this file hand-maintained
-- ten near-identical arms that differed only in the @RX_*@ regex and the kw_aspect filter, and
-- the point of the move is that a lexicon edit now moves all ten at once instead of relying on
-- ten hand-edits staying in step. The expansion is verified byte-identical to the hand-written
-- arms' rendered output, so nothing below the placeholder changed.
