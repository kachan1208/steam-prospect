 ...
 * Pass a glossary `term` and everything but `worked` and `sentinel` comes from
 * lib/glossary.ts; explicit props override the glossary field by field.
 */
...
export interface InfoTipContent {
  ...
  source?: ReactNode;
  /** Caveats — defaults to the glossary's notes. */
  notes?: ReactNode;
}

export interface InfoTipProps extends Partial<InfoTipContent> {
...
/** Resolve the panel content from a glossary term plus explicit overrides. */
export function resolveInfoTip(props: Pick<InfoTipProps, keyof InfoTipContent | "term" | "hideNotes">): InfoTipContent | null {
...
export function InfoTip(props: InfoTipProps) {
  const { ariaLabel, workedLabel = "With these numbers", className, children } = props;
  const content = resolveInfoTip(props);
...
export interface MetricExplainProps {
  ...
  help?: string;
  /** This row's real numbers substituted into the formula. */
  worked?: ReactNode;
