...
- Trend verdict language everywhere: ▲/▼ + signed % over 90 days. Up = accent-300, down/flat = paper 55%. Never red/green — the palette is mono steel.
  - **AMENDED 2026-08-27 — sanctioned exception: radar verdict rings.** The mono-steel rule above still governs trend verdicts and aspect-sentiment bars on every surface, but the Radar board's five VERDICT rings are hue-coded on purpose (user directive: "add some colors so it's easy to understand where to focus"): green = enter, steel = watch, violet = emerging, amber = crowded, terracotta = declining. Shipped as `--verdict-enter/-crowded/-declining/-emerging` in `web/src/index.css` (both modes) and consumed by `RadarBoard.tsx`'s `RING_FILL`; the matching note lives in `web/src/lib/palette.ts`'s module docstring, so code and handoff agree. The exception is narrow and conditional: color is **reinforcement only** — plate position, fill style (hollow = team-scale), the dashed emerging halo and a legend that prints each verdict's WORD beside its swatch all carry the meaning without hue, so it survives grayscale and CVD. Enter-green and the warm crowded/declining family are separated by lightness as well as hue; alarm-red is deliberately avoided (crowded must read "caution", never "error"); gold was tested for enter and rejected for colliding with amber under deuteranopia. Do not extend hue-coding past the radar's verdict vocabulary without the same checks.
